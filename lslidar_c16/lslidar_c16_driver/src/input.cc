...
int InputSocket::getPacket(lslidar_c16_msgs::LslidarC16Packet* pkt, const double time_offset)
{
 ...
 // while (true)
  ...
  getPacketTimeStamp(*pkt);

  // Average the times at which we begin and end reading.  Use that to
  // estimate when the scan occurred.
  double time2 = ros::Time::now().toSec();
  if(time_synchronization_){
      pkt->stamp = this->timeStamp;
  } else{
      pkt->stamp = ros::Time((time2 + time1) / 2.0 );
      //ROS_INFO("timestamp: %f",pkt->stamp.toSec());
  }
...
void InputSocket::getPacketTimeStamp(lslidar_c16_msgs::LslidarC16Packet &packet) {
    ...
    if(head2[0] == 0xa5 && head2[1] == 0xff && head2[2] == 0x00 && head2[3] ==0x5a ){
        if(head2[4] == 0x11 && head2[5] == 0x11 && head2[6] == 0x55 && head2[7] ==0x55 ){
            int version_data = packet.data[1202];
            if(2 == version_data) {
            ...
            GPS_ts = this->pointcloudTimeStamp;
        }
    }
